...
Point calcul_pos_init(Image I)
{
    ...
    for(i = 1 ; i <= I.la_largeur_de_l_image ; i++)
    {
        for (j = 1 ; j <= I.la_hauteur_de_l_image ; j++)
        {
            p = get_pixel_image(I, j, i);
            p_1 = get_pixel_image(I, j, i-1);
...
Orientation calcul_orient(Image I, Robot R)
{
    ...
    //devant gauche te droit depandent de l'orientation du robot 
    switch(R.o)
    {
        case NORD:
