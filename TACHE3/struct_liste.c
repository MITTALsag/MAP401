...
Tableau_Point cree_Tableau_point_vide (unsigned int n)
{
	...
	if (T.tab == NULL)
		ERREUR_FATALE( "sequence_points_liste_vers_tableau :  allocation impossible\n");
        return T;
}
...
Liste_Point concatener_liste_Point(Liste_Point L1, Liste_Point L2)
{
	...
	L2.first->precedent = L1.last; /*le precendent du premier elem de L2 est
le dernier elem de L1 */
...
Liste_Point supprimer_premier_element_liste_Point(Liste_Point L)
{
	...
	if (e2 == NULL)    /* cas d'une liste L avec un seul element */
		...
		e2->precedent = NULL; /* e2 != NULL donc il y avait une valeur
dans precendant mais comme e2 devient le premier alors plus de precedent */
	L.taille--;
	return L;
}
